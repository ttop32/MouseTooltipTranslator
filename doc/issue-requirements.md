...
| [#88](https://github.com/ttop32/MouseTooltipTranslator/issues/88) | 2024-02-08 | 브라질 포르투갈어 | ✅ 적용 | 번역 대상 언어에 `pt-BR`/`pt-PT` 추가(`langList`) + pt-BR 로케일. 실측: google·bing 모두 pt-BR/pt-PT 어휘를 정확히 구분(celular/banheiro/ônibus ↔ telemóvel/casa de banho/autocarro). yandex는 pt-BR만(pt-PT 요청 실패), deepl은 두 변종 모두 실패 — **엔진 자체가 미지원**(무료 jsonrpc는 `PT`만 받음. `PT-BR`/`PT-PT`로 보내도 거부됨을 실측). 즉 지원 언어 집합이 엔진마다 다른데 `langList`는 전 엔진 공용이라, 미지원 조합은 100% 실패 후 fallback 엔진으로 스왑됨(0.1.247에서 언어쌍 단위 쿨다운으로 완화). 같은 이유로 bing은 자바어(`jw`/`jv` 둘 다) 미지원 |
...
| [#257](https://github.com/ttop32/MouseTooltipTranslator/issues/257) | 2025-09-03 | 브라질 포르투갈어 | ✅ 적용 | `langList`에 pt-BR/pt-PT. 후속(#363): 감지기가 유럽식 어휘엔 `pt-PT`, 브라질식엔 bare `pt`를 반환하는 비대칭 때문에 pt↔pt-BR이 동일 언어 취급됨 → `isSameLanguage`로 중복 툴팁 차단하되, `isRedundantTranslation`으로 **결과 텍스트가 실제로 바뀐 경우엔 계속 표시**(pt→pt-PT 변환 유지). auto 감지로 변종을 강제하려면 Translate From을 명시 지정 |
...
| [#338](https://github.com/ttop32/MouseTooltipTranslator/issues/338) | 2026-06-16 | 재시작 후 언어별 음성 속도 초기화 | ✅ 수정 | `loadStorageData`가 기본 스키마 키만 fetch해 동적 키(`ttsRate_<lang>`/`ttsVoice_<lang>`)가 재시작 시 복원 안 됨 → `storage.local.get(null)`로 전체 키 로드 |
| [#339](https://github.com/ttop32/MouseTooltipTranslator/issues/339) | 2026-06-17 | 0.1.231 PDF 뷰어 deepmerge 에러 | ✅ 수정 | PR #335이 doq prefs.js에 bare `import deepmerge`. doq는 PDF.js 뷰어에서 비번들 raw ES 모듈로 로드돼 specifier 못 풂 → 인라인 `deepMerge`로 교체 |
...
| [#352](https://github.com/ttop32/MouseTooltipTranslator/issues/352) | 2026-07-15 | AltGr(국제 키보드)가 단축키 오발동 | ✅ 수정 | Windows에서 AltGr(우측 Alt, 스위스/독일/프랑스/폴란드 등 레이아웃)는 브라우저가 `Ctrl+AltRight`+`getModifierState("AltGraph")=true`로 디스패치. 확장의 우 Alt = 글쓰기-번역 단축키(`AltRight`)라, `@ € { } [ ] \ \| ~` 등 특수문자 **입력** 시 글쓰기-번역/선택이 오발동해 텍스트를 두 번 하이라이트하고 공백 삽입. **수정**: `contentScript.js handleKeydown` 맨 앞에서 `e.getModifierState("AltGraph")`면 early-return해 단축키 처리를 건너뜀(입력으로 취급). keyup은 그대로 두어 키 stuck 없음. 보고자 MrDx3 제안 반영. **#355로 회귀 → 0.1.243에서 옵션화**. **0.1.244 (PR #358 머지)**: AltGr의 **ControlLeft 절반은 자동 판별** 추가. AltGr는 항상 `ControlLeft`→`AltRight` 순서라, `handleKeydown`에서 ControlLeft를 20ms(`CTRL_VERIFICATION_DELAY`) 지연 등록하고 그 사이 AltRight가 오면 취소(=AltGr). 진짜 왼쪽 Ctrl은 뒤에 AltRight가 안 붙어 정상 등록. 옵션 불필요(TTS 기본키가 ControlLeft라 이걸로 자동 보호). **AltRight 절반**은 물리키가 곧 AltGr라 자동 판별 불가 → `ignoreAltGr` 옵션 유지. PR #358은 stale base로 머지돼 0.1.244 Tooltip Placement를 되돌리고 `ignoreAltGr` 처리를 삭제했으나 후속 커밋에서 **둘 다 복구**(placement 로직 재적용, AltGraph early-return 재추가), 죽은 `shouldVerifyKey`·주석처리 구코드 정리. **0.1.245: defer 취소 조건 강화** — MR 버전은 `AltRight`면 **무조건** pending ControlLeft를 취소해, 실제 `Left Ctrl + (AltGr 아닌) Right Alt` 조합에서 진짜 Ctrl을 잘못 버렸음. 취소를 `e.getModifierState("AltGraph")===true`일 때로 게이트 → **진짜 AltGr일 때만** phantom 취소, 의도적 Ctrl+RightAlt 조합은 보존. phantom ControlLeft는 one-shot 동작(TTS 발화·글쓰기번역)이라 발동 후 되돌릴 수 없어 사후정리 대신 **사전 defer(차단)** 유지가 정답. keyBindings 디스패치 리팩터 유지. ⚠ AltGr는 실제 브라우저 검증 필요(헤드리스 재현 불가) |
| [#355](https://github.com/ttop32/MouseTooltipTranslator/issues/355) | 2026-07-23 | #352 회귀: 우측 Alt(AltGr) 트리거 죽음 | ✅ 수정 | #352 fix가 AltGraph 활성 시 **무조건** early-return → 우측 Alt를 트리거로 쓰던 국제 레이아웃 사용자(그 레이아웃에선 우 Alt 물리키 = AltGr)의 단축키가 완전히 죽음(summerrelax·odorizzioficial 보고). summerrelax 요청대로 **옵션화**: `setting_default.js`에 `ignoreAltGr`(toggle, 기본 `"false"`, i18nKey `Ignore_AltGr_Modifier`, keyboard 탭) 추가. `handleKeydown`의 early-return을 `setting["ignoreAltGr"] === "true"`일 때만 발동하도록 게이트. 기본 Off = #352 이전 동작 복원(우 Alt 트리거 정상), AltGr 특수문자 입력이 오발동하는 사용자만 On. 로케일 55개 `Ignore_AltGr_Modifier` 추가(insertLocale.py) |
| [#353](https://github.com/ttop32/MouseTooltipTranslator/issues/353) | 2026-07-23 | #352 회귀: 키 눌러도 글쓰기 번역 안 됨(Edge·Chrome) | ✅ 수정(#355 동일) | odorizzioficial 보고 + summerrelax 댓글 *"대부분 사용자는 true RightAlt가 없고 AltGr만 있으며 AltGr+1234를 쓸 일이 없다 → 그들에겐 확장이 깨진 상태"* = #352/#355와 완전히 동일 원인. 0.1.243 `ignoreAltGr` 기본 Off로 자동 해소(우 Alt 트리거 복원). 별도 코드 변경 불필요 |
| [#354](https://github.com/ttop32/MouseTooltipTranslator/issues/354) | 2026-07-23 | 우 Alt 오작동 + 번역 느림/"Bing·Google broken" | 🔶 부분수정 | (a) **키보드**: 우 Alt 안 됨 + "Left Ctrl로 바꾸니 Left Ctrl·Right Alt 둘 다 트리거" → Windows AltGr = `Ctrl+AltRight`라 Ctrl로 바꿔도 AltGr가 Ctrl 이벤트를 쏘는 것. #352 회귀와 동일 → 0.1.243 `ignoreAltGr` 기본 Off로 우 Alt 트리거 복원돼 해소. **0.1.244**: "Left Ctrl로 바꾸니 Left Ctrl·Right Alt 둘 다 트리거"(AltGr가 Ctrl 이벤트를 쏘던 것)는 ControlLeft 20ms 지연-취소로 자동 해소. **0.1.245**: 그 취소를 `getModifierState("AltGraph")` 확인 시에만 발동하도록 강화(#352 참조) — 진짜 AltGr만 phantom Ctrl 취소, 의도적 Ctrl+RightAlt 조합은 진짜 Ctrl 보존. (b) **번역 "broken"/느림**: `src/translator`·`background.js`는 0.1.235 이후 무변경(회귀 창 밖) → 코드 회귀 아님. 구글/빙 IP rate-limit(서비스측)이며, #352 오발동이 번역요청을 연타해 rate-limit을 **악화**시킨 2차 증상으로 추정 → 키 오발동 해소가 간접 완화. 재현 불가·서비스 의존이라 별도 코드 조치 없음 |
