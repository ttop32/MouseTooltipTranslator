...
| [#354](https://github.com/ttop32/MouseTooltipTranslator/issues/354) | 2026-07-23 | 우 Alt 오작동 + 번역 느림/"Bing·Google broken" | 🔶 부분수정 | (a) **키보드**: 우 Alt 안 됨 + "Left Ctrl로 바꾸니 Left Ctrl·Right Alt 둘 다 트리거" → Windows AltGr = `Ctrl+AltRight`라 Ctrl로 바꿔도 AltGr가 Ctrl 이벤트를 쏘는 것. #352 회귀와 동일 → 0.1.243 `ignoreAltGr` 기본 Off로 우 Alt 트리거 복원돼 해소. (b) **번역 "broken"/느림**: `src/translator`·`background.js`는 0.1.235 이후 무변경(회귀 창 밖) → 코드 회귀 아님. 구글/빙 IP rate-limit(서비스측)이며, #352 오발동이 번역요청을 연타해 rate-limit을 **악화**시킨 2차 증상으로 추정 → 키 오발동 해소가 간접 완화. 재현 불가·서비스 의존이라 별도 코드 조치 없음 |
