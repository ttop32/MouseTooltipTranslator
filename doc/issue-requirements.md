...
| [#352](https://github.com/ttop32/MouseTooltipTranslator/issues/352) | 2026-07-15 | AltGr(국제 키보드)가 단축키 오발동 | ✅ 수정 | Windows에서 AltGr(우측 Alt, 스위스/독일/프랑스/폴란드 등 레이아웃)는 브라우저가 `Ctrl+AltRight`+`getModifierState("AltGraph")=true`로 디스패치. 확장의 우 Alt = 글쓰기-번역 단축키(`AltRight`)라, `@ € { } [ ] \ | ~` 등 특수문자 **입력** 시 글쓰기-번역/선택이 오발동해 텍스트를 두 번 하이라이트하고 공백 삽입. **수정**: `contentScript.js handleKeydown` 맨 앞에서 `e.getModifierState("AltGraph")`면 early-return해 단축키 처리를 건너뜀(입력으로 취급). keyup은 그대로 두어 키 stuck 없음. 보고자 MrDx3 제안 반영 |
