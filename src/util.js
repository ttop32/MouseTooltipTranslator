...
export function caretRangeFromPointOnShadowDom(x,y){
    ...
    var textNodes=shadows.map(shadow=>Array.from(getAllTextNodes(shadow))).flat();
    var textNodes=textNodes.filter(textNode=>checkXYInElement(getTextRange(textNode),x,y));
    var ranges=textNodes.map(textNode=>Array.from(getCharRanges(textNode))).flat();
